...
class RxCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    ...
    if (first_part >= 0) {
      ...
      if (a2wToWrite > first_part) memcpy(a2wRing, data + first_part, a2wToWrite - first_part);
      a2wHead = (a2wHead + a2wToWrite) & A2W_RING_SIZE_SUB_ONE;
      DEBUG_PRINTLN(ringLack > 0    ? "填满，丢弃：" + String(ringLack) + " B"
                    : ringLack == 0 ? "正好填满"
...
void airBle_update() {
  ...
  if (deviceConnected) {  // 只在有连接时更新手动w2a缓存
