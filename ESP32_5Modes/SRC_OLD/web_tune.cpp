...
void WebConfig::begin() {
    ...
    // 6. 启动 mDNS — STA 模式下可通过 esp-web.local 访问
    if (MDNS.begin("esp-web")) {
        MDNS.addService("http", "tcp", 80);
        DEBUG_PRINTLN("[WebConfig] mDNS started: esp-web.local");
    } else {
...
void WebConfig::handleNotFound() {
    ...
    // esp-web.local 通过 mDNS 在 STA 模式下可用
    if (host != "10.0.0.1" && host != "esp-web.local") {
